...
(function ($) {
    ...
    function isInteger(obj) {
    	 if(!(obj%1 === 0)){
    		   Ext.Mytip.msg('提示', obj+"不是正整数，请您填写正确的整数！"); 
    	 }
    	 return !(obj%1 === 0);
    	}
    ...
    fn.Initialization = function () {
        ...
        // 上一月
        $(this.opts.menu_el).on('click', '.to-month', function () {
            me._toMonth($(this));
        });
     ...
     * @private
     */
    fn._toMonth = function (obj) {
        var y = Number(formatDate(new Date(), 'yyyy'));
        var m = Number(formatDate(new Date(), 'MM'));
        var month = Number(obj.attr('data-month'));
        //obj.addClass('active').siblings().removeClass('active');
        if (month >= 0) {
            //var month = (m + index) % 12;
            //month = month == 0 ? 12 : month;
            y = Number(month < m ? (y + 1) : y);
            this.month = this.dateToObject(y + '/' + month);
            this.createCalendar();
        }
    };
    /**
   ...
   **/
    fn.createMenu = function () {
        ...
        for (var i = 0; i <= lastMonth; i++) {
            var month = (date.getMonth() + i + 1) % 12;
            month = month == 0 ? 12 : month;
            str += '<p id="menu_month_' + month + '" class="to-month" data-month="' + month + '">' + month + ' 月</p>';
        }
        var obj = $(this.opts.menu_el);
        obj && obj.html(str);
    };
    /**
     * 创建日历
    ...
    fn.createCalendar = function () {
        //修改menu
        var _month =Number(formatDate(this.month, 'MM'));
        $("#menu_month_" + _month).addClass('active').siblings().removeClass('active');
    ...
    fn._createDaySetupInputGroup = function () {
        ...
        for (var i = 0; i < config.length; i++) {
            ...
            if (val.type == "radio") {
                for (var j = 0, _len = val.items.length; j < _len; j++) {
                    html += '<label class="check"><input class="checked" name="' + val.key + '" type="radio" value="' + val.items[j].value + '">' + val.items[j].text + '</label>';
                }
            } else if (val.type == "checkbox")
            {
                for (var j = 0, _len = val.items.length; j < _len; j++) {
                    html += '<label class="check"><input class="checked"  name="' + val.key + '" type="checkbox" value="' + val.items[j].value + '">' + val.items[j].text + '</label>';
                }
            }
            else if (val.type == "select")
            {
            	html+='<select name="'+ val.key + '">';
                for (var j = 0, _len = val.items.length; j < _len; j++) {
                	html+='<option value="' + val.items[j].value + '">' + val.items[j].text + '</option>';
                }
                html+='</select>';
            }
            else {
                html += '   <input name="' + val.key + '" type="number">';
            }
            html += '</li>';
        }
        return html;
    ...
    fn.setupPriceAndStock = function () {
        ...
        $(this.opts.el).on('click', '.valid-hook', function () {
            ...
            $setContainer.find('.cddsw-form-wrapper [type="radio"]').each(function(){
            	this.checked=false;
            });
            $setContainer.find('.cddsw-form-wrapper [type="checkbox"]').each(function(){
            	this.checked=false;
            });
            $setContainer.find('.cddsw-form-wrapper select').val(0);
            $setContainer.find('[name="enableDateRange"]').prop('checked', false);
            $setContainer.find('[name="setWeek"]').prop('checked', false);

            // 用户传入字段
            $.each(data, function (key, val) {
            	var _input= $setContainer.find('[name="'+ key +'"]');
            	if(_input.length>1&&(_input[0].type=="radio"||_input[0].type=="checkbox")){
...
/*
        ...
        $('body').on('click', '.capricorncd-date-detailed-settings .btn-confirm', function () {
            ...
            //库存
            var  stock= $price.find('[name="stock"]').val();
            // 门市价
            var  priceMarket= $price.find('[name="priceMarket"]').val();
            //最小购买数
            var  buyNumMin= $price.find('[name="buyNumMin"]').val();
            //最多购买数
            var buyNumMax =$price.find('[name="buyNumMax"]').val();
             //指导价【本系统中的微信金额】
            var  price= $price.find('[name="price"]').val();
            //度假币金额
            var  djbprice= $price.find('[name="djbprice"]').val();
            //结算价
            var  priceSettlement= $price.find('[name="priceSettlement"]').val();
            //度假币限制
            var  priceRetail= $price.find('[name="priceRetail"]').val();
            
            if(startDay==''||endDay==''||stock==''||priceMarket==''
            	||buyNumMin==''||buyNumMax==''||price==''||djbprice==''||priceSettlement==''){
            	  Ext.Mytip.msg('提示', "您还有没有填写的数据！"); 
            	  return;
            }
            if(isInteger(stock)||isInteger(priceMarket)||isInteger(buyNumMin)||isInteger(buyNumMax)||isInteger(buyNumMin)||isInteger(buyNumMax)||isInteger(price)||
                	isInteger(djbprice)||isInteger(priceSettlement)){
            	 //   Ext.Mytip.msg('提示', "请您填写正确的整数！"); 
                	  return;
                }
