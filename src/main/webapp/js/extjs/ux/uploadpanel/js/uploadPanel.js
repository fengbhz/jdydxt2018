...
UploadPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		//alert(config.completeFileHander);
		//this.completeFileHander = config.completeFileHander;
		this.progressInfo = {
			filesTotal : 0,
			filesUploaded : 0,
			bytesTotal : 0,
...
//			}
		...
		UploadPanel.completeFileHander = this.completeFileHander;
...
//		});
	...
	formatState : function(_v, cellmeta, record) {
		...
		switch (_v) {
			case SWFUpload.FILE_STATUS.QUEUED :
				...
				//returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId+ '" />【X】</span>';
				break;
	...
	onUploadComplete : function(file) {
		if (this.getStats().files_queued > 0 && this.uploadStopped == false) {
			...
			if(UploadPanel.completeFileHander ){
				eval(UploadPanel.completeFileHander).call();
			} else {
				alert('未指定上传完成回调函数！');
			}
			Ext.Msg.hide();
...
UploadPanel.completeFileHander = null;
